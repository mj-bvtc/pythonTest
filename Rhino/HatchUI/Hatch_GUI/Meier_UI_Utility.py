...
class UIPanel(FlowLayoutPanel):
    ...
    def addBlankLabel(self,Length,breakFlowAfter=False):
        """
        Adds a blank Label control.
        Parameters:
    ...
    def addLabel(self, name, text, color, breakFlowAfter,width=None):
        """
    ...
    def addListBox(self,name,size,items,breakFlowAfter):
        lb=ListBox()
        lb.Name=name
        if items==[]:
            lb.Items.Add("[None]")
            ...
            for item in items:
                lb.Items.Add(item)
        if size:
            lb.Size=System.Drawing.Size(size[0], size[1])
        else:
            lb.AutoSize=True
        
        lb.Margin = System.Windows.Forms.Padding(3, 5, 3, 0)
        self.SetFlowBreak(lb, breakFlowAfter)
    ...
    def addMonthCalendar(self, name, days=7, delegate=None, breakFlowAfter=True):
        c = System.Windows.Forms.MonthCalendar()
        c.Name=name
        c.MaxSelectionCount=days
        if (delegate != None):
    ...
    def addPictureBox(self, name, filename, breakFlowAfter=True):
        """
    ...
    def addRadioButton(self, name, text, checked, breakFlowAfter, delegate):
        """
        Adds a RadioButton control to the panel.
        Parameters:
            name: The name for the control. You may pass "" if you don't need to
                reference the control by name. 
            text: The label for the control. 
            checked: Pass True to initial set the box to checked; otherwise False.
            breakFlowAfter: Pass True to have the next control on a new line. 
                False to have the next control on the same line if there is room.
            delegate: The event handler when the CheckStateChanged event is raised. 
                Pass None if you don't need to handle the event. 
    ...
    def addButton(self, name, text, width, filename, breakFlowAfter, delegate):
        """
        ...
        Parameters:
            ...
            width: The horizontal size in pixels for the control.
            filename: The filename for the button icon to display.
                Examples: "c:/okay.png" or "./Resources/Icons/Iconsreset.jpg"
            breakFlowAfter: Pass True to have the next control on a new line. 
                False to have the next control on the same line if there is room.
        ...
        if (text == "OK") or (text =="Done") or (text=="Update"):
    ...
    def addNumericUpDown(self, name, lowerLimit, upperLimit, increment,decimalPlaces, initValue, width, breakFlowAfter, delegate):
        """
    ...
    def addPropertyGrid(self, name, width, height, object, breakFlowAfter, delegate):
        """
        Adds a property grid control. This control will automatically display
            and allow the user to edit all the properties of the class instance
            passed (object).
        Parameters:
            name: The name for the control. You may pass "" if you don't need to
                reference the control by name. 
            width: The horizontal size in pixels for the control.
            object: The instance of the class to edit. 
            breakFlowAfter: Pass True to have the next control on a new line. 
                False to have the next control on the same line if there is room.
        Returns:
            The control which was added.
        """
        c = System.Windows.Forms.PropertyGrid()
        c.ToolbarVisible = False
        c.HelpVisible = False
        c.PropertySort = System.Windows.Forms.PropertySort.Alphabetical
        c.Name = name
        c.SelectedObject = object
        c.Size = System.Drawing.Size(width, height)
        c.TabIndex = self.tabIndex; self.tabIndex += 1
        if (delegate != None):
            c.PropertyValueChanged += delegate
        self.SetFlowBreak(c, breakFlowAfter)
        self.controls.append(c)
    
    def addIndent(self, size):
        """
        Adds an indent of the specified size. This is a Label control with
            the specified number of spaces. This is shorter notion to simplify 
            formatting.
            Note: Even an addIdent(0) will indent some because of padding. 
        Parameters:
            size: The number of spaces to indent
        Returns:
            The control which was added.
        """
        c = Label()
        ...
        c.AutoSize = True
        c.Margin = System.Windows.Forms.Padding(3, 5, 3, 0) # 5 is to align with other controls vertically
        self.SetFlowBreak(c, False)
        self.controls.append(c)
        return c
    
    def addNewline(self):
        """
        Adds an blank label with flow break set to true to provide vertical 
            spacing like a new line.
        Returns:
            The control which was added.
        """
        c = Label()
        c.Text = " "
        c.AutoSize = True
        c.Margin = System.Windows.Forms.Padding(0, 0, 0, 0) # 5 is to align with other controls vertically
        self.SetFlowBreak(c, True)
        self.controls.append(c)
        return c
